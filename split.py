...
def split_reg(region):
    seed = 52
    folder_dir = f"/mnt/disk3/tunm/Subseasonal_Forecasting/data2/data6789_reg_{region}_seed{seed}"
    os.makedirs(folder_dir, exist_ok=True)
...
def split_train_valid_reg(region):
    ...
    for index, row in data.iterrows():
        ...
        for year in years:
            for lead_time in lead_times:
                path_ecmwf = f"/mnt/disk3/longnd/env_data/S2S_0.125_old/S2S_0.125/nparr_reg_{region}/Step24h/{date}.npy"
                expanded_data.append({
                    "pathECMWF": path_ecmwf,
                    "leadTime": lead_time,
                    "year": year,
    ...
    valid_data = data[data["year"].isin([2014, 2015, 2016, 2017])]

    # Take data from 2002 to 2017 as remaining data
    train_data = data[~data["year"].isin([2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021])]
...
def splitv2_train_valid_reg(region):
    seed = 52
    folder_dir = f"/mnt/disk1/tunn/Subseasonal_Prediction/data4/data6789_reg_{region}_seed{seed}"
    os.makedirs(folder_dir, exist_ok=True)
